 ...
 * 基于javascript ES6模块化开发，提供灵活的配置项，后端分页数据格式可以自定义配置，主题可自由发挥，
...
const setLoadingStyle = (containerElement) => {
    containerElement.setAttribute('style', X_PAGE_STYLES.loading);
};
...
const setBtnStatus = (elements, pageMeta) => {
    let setStyle = (element, value) => {
        ...
        if (value !== '') {
            ...
            element.setAttribute('style', prevStyle.replace(value, ''));
        }
    };
    const elms = elements;
    const disabled = X_PAGE_STYLES.disabled;
    if (pageMeta.pages === 1) {
 ...
 * @type {any}
 */
export const defaultSetting = deepClone(X_PAGE_DEFAULT_SETTING);
/**
 * 默认选项
 * @type {any}
 */
export const defaultOption = deepClone(X_PAGE_DEFAULT_OPTION);

/**
 * 分页控件
 */
export class xpage {
    constructor(containerId, setting) {
        this.mode = 'local'; //默认为静态分页
        this.pageMetaData = {}; //返回的分页元数据
        this.containerId = containerId;
        this.container = document.getElementById(containerId);
        this.setting = deepObjectAssign(deepClone(X_PAGE_DEFAULT_SETTING), setting);
        ...
        this.container.setAttribute('style', X_PAGE_STYLES.disabled);
        this.subElements = {
     ...
     * @see defaultSetting
     */
    static init(containerId, setting) {
        return new xpage(containerId, setting);
    }
